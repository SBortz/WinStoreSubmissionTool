...
namespace WinStoreSubmissionTool
{
	...
	public class IngestionClient : IDisposable
	{
		...
		public static async Task UploadFileToBlob(string fileName, string sasUrl)
		{
			...
			int bufferSize = (int)totalBytes / 100;

			using (Stream stream = new FileStream(fileName, FileMode.Open))
			{
				var blockBob = new CloudBlockBlob(new Uri(sasUrl));
				
				using (var blockWriteStream = await blockBob.OpenWriteAsync())
				{
					byte[] buffer= new byte[bufferSize];
					int offset = 0;
					int actualSize = 0;

					while ((actualSize = await stream.ReadAsync(buffer, 0, buffer.Length)) > 0)
					{
						await blockWriteStream.WriteAsync(buffer, 0, actualSize);
						Console.Write(".");
					}
				}
			}
